...
def validate_n(p, allowed):
    d=p["decision"]; units=list(dict.fromkeys(p["existing_unit_ids"]))
    if any(u not in allowed for u in units): raise ValueError("invalid unit")
    title=clean(p["candidate_title"]); desc=clean(p["candidate_description"])
    if d=="existing_unit":
        if not units or title or desc: raise ValueError("bad existing_unit payload")
    elif d in {"new_subunit_candidate","new_major_chapter_candidate"}:
        if units or not title or not desc: raise ValueError("bad candidate payload")
    elif d=="out_of_scope":
        if units or title or desc: raise ValueError("bad OOS payload")
    else: raise ValueError("bad decision")
    return {"decision":d,"existing_unit_ids":units,
            "candidate_title":title,"candidate_description":desc,
            "confidence":p["confidence"],"rationale":clean(p["rationale"])}
