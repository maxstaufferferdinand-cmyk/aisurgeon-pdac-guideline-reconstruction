...
class NcbiClient:
    ...
    def _post(self, endpoint: str, data: dict[str, str]) -> requests.Response:
        ...
        while True:
            ...
            try:
                ...
                # A premature/truncated HTTP body (ChunkedEncodingError) is a
                # transport failure, not a permanent query error. Treat it the
                # same way as timeouts/connection drops so unattended runs can
                # recover automatically.
                self._last_request_at = time.monotonic()
                wait_seconds = self.transient_retry_seconds
                print(
                    f"  WARN: transient NCBI network/stream error on {endpoint} "
    ...
    def efetch_xml(self, pmids: Sequence[str]) -> bytes:
        ...
        while True:
            ...
            # A 200 response can still be unusable if a proxy/server truncates
            # the body. Empty or non-well-formed XML is therefore treated as a
            # transient response and the same PMID batch is retried after the
            # configured wait instead of aborting the full run.
            try:
                if not content.strip():
                    raise ET.ParseError("empty EFetch response")
                ET.fromstring(content)
            except ET.ParseError as exc:
                wait_seconds = self.transient_retry_seconds
                print(
                    f"  WARN: transient invalid/truncated EFetch XML "
                    f"({type(exc).__name__}: {exc}); attempt {attempt}; "
                    f"retrying same PMID batch in {wait_seconds:.0f}s",
                    flush=True,
                )
                time.sleep(wait_seconds)
                continue
